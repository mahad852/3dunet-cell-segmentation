...
def get_mito_masks_custom(imgs: np.ndarray):
    ...
    for i, img in enumerate(imgs):
        for z in range(len(img[0])):
            masks[i][0][z] = img[0][z] >= np.percentile(img[0][z], 97)
    return torch.Tensor(masks)
...
def main():
    ...
    val_ds = CellDataset(data_path='/home/mali2/datasets/CellSeg/Widefield Deconvolved/Microtubule Channel', num_channels=1, transform_image=val_imtrans, transform_seg=val_segtrans, is_segmentation=False, is_train=False)
    ...
    with torch.no_grad():
        ...
        for val_data in val_loader:
            ...
            for output, path in zip(val_outputs.detach().cpu(), img_pths):
                ...
                tifffile.imwrite(out_file, denoise_img(scale_image(output[0]).cpu().numpy()).astype(np.uint16))
